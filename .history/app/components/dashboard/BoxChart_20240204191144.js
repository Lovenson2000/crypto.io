...
export default function ChartWrapper({ coins }) {

    return (
        <div className='grid place-items-center place-content-center grid-cols-1 sm2:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
            {
                coins.map((coin) => (
                    <BoxChart
                        coin={coin}
...
function BoxChart({ coin }) {
    return (
        <>
            <div className="md1:bg-red-400 bg-white w-52 p-3 text-slate-700 relative border-b-2 border-main m-4 shadow-md rounded-sm md2:bg-red-400">
                <Image
                    width={30}
                    height={30}
                    src={coin.image}
