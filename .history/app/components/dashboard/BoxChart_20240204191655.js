...
export default function ChartWrapper({ coins }) {
    ...
    return (
        <div className='grid grid-cols-1 place-items-center place-content-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-'>
            {
                coins.map((coin) => (
                    <BoxChart
                        coin={coin}
...
function BoxChart({ coin }) {
    return (
        <>
            <div className="bg-white w-52 p-3 text-slate-700 relative border-b-2 border-main m-4 shadow-md rounded-sm md:w-60">
                <Image
                    width={30}
                    height={30}
                    src={coin.image}
