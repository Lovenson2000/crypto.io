import { fetchCoins, fetchFirstFourCoins } from '@/app/api/data';
...
export default async function ChartWrapper() {
    const response = await fetchFirstFourCoins().sort((a,b) => b.current_price - a.current_price);
    const coins = response.slice(0, 4);
    

    return (
        <div className='grid grid-cols-1 place-items-center place-content-center md:grid-cols-2 lg:grid-cols-4 gap-'>
            {
                coins.map((coin) => (
                    <BoxChart
                        coin={coin}
...
function BoxChart({ coin }) {
    return (
        <div className="bg-white w-64 p-3 text-slate-700 relative border-b-2 border-main m-4 shadow-md rounded-sm">
            <Image
                width={30}
                height={30}
                src={coin.image}
