...
import { pickFirstNPrices } from "@/app/lib/utils";

export default function Page() {
  const prices = bitcoinaData.sparkline_in_7d.price;
  const firstTenPrices = pickFirstNPrices(prices, 10);
  const roundedPrices = firstTenPrices.map((price) => parseFloat(price.toFixed(4)));
